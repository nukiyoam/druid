...
public class WallFilter extends FilterAdapter implements WallFilterMBean {
    ...
    public PreparedStatementProxy connection_prepareStatement(FilterChain chain, ConnectionProxy connection, String sql,
        ...
        try {
            ...
            PreparedStatementProxy stmt =
                    chain.connection_prepareStatement(connection, sql, resultSetType, resultSetConcurrency);
            setSqlStatAttribute(stmt);
            return stmt;
        } finally {
            WallContext.clearContext();
    ...
    public CallableStatementProxy connection_prepareCall(FilterChain chain, ConnectionProxy connection, String sql,
        ...
        try {
            ...
            CallableStatementProxy stmt =
                    chain.connection_prepareCall(connection, sql, resultSetType, resultSetConcurrency);
            setSqlStatAttribute(stmt);
            return stmt;
        } finally {
            WallContext.clearContext();
    ...
    private void wallUpdateCheck(PreparedStatementProxy statement) throws SQLException {
        ...
        List<WallUpdateCheckItem> wallUpdateCheckItems =
                (List<WallUpdateCheckItem>) statement.getAttribute(ATTR_UPDATE_CHECK_ITEMS);
        if (wallUpdateCheckItems != null) {
            for (WallUpdateCheckItem item : wallUpdateCheckItems) {
                ...
                boolean valid =
                        config.updateCheckHandler.check(item.tableName, item.columnName, setValue, filtersValues);
                if (!valid) {
    ...
    public String check(String sql) throws SQLException {
        return checkInternal(sql).getSql();
    }
    ...
    private WallCheckResult checkInternal(String sql) throws SQLException {
        ...
        if (violations.size() > 0) {
            ...
            if (isLogViolation()) {
                LOG.error("sql injection violation, dbType " + getDbType() + ", druid-version "
                        + VERSION.getVersionNumber() + ", " + firstViolation.getMessage() + " : " + sql);
            }
            ...
            if (throwException) {
                if (violations.get(0) instanceof SyntaxErrorViolation) {
                    ...
                    throw new SQLException("sql injection violation, dbType " + getDbType() + ", druid-version "
                            + VERSION.getVersionNumber() + ", " + firstViolation.getMessage() + " : " + sql);
                }
            }
        }
    ...
    @Override
    public <T> T resultSet_getObject(FilterChain chain, ResultSetProxy resultSet, int columnIndex, Class<T> type)
            throws SQLException {
    ...
    public boolean resultSetMetaData_isAutoIncrement(FilterChain chain, ResultSetMetaDataProxy metaData, int column)
            throws SQLException {
        return chain.resultSetMetaData_isAutoIncrement(metaData,
    ...
    public boolean resultSetMetaData_isCaseSensitive(FilterChain chain, ResultSetMetaDataProxy metaData, int column)
            throws SQLException {
        return chain.resultSetMetaData_isCaseSensitive(metaData,
                metaData.getResultSetProxy().getPhysicalColumn(column));
    }
